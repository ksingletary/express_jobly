...
router.post("/:username/jobs/:id", authenticateJWT, ensureUserExists, ensureCorrectUserOrAdmin, async function (req, res, next) {
  try {
    const jobId = +req.params.id;
    if (isNaN(jobId)) throw new BadRequestError("Job ID must be a number");
    const job = await User.applyForJob(req.params.username, jobId);
